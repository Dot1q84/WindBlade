...
class HighlightView {
    ...
    /**裁剪框四边上的圆点显示时机（改变时显示，一直显示，不显示）*/
    enum HandleMode { Changing, Always, Never }
    /**裁剪框内的网格显示时机（改变时显示，一直显示，不显示）*/
    enum GridMode {Changing, Always, Never}
...
//    private boolean showThirds;
    ...
    private GridMode gridMode = GridMode.Changing;
...
//            showThirds = attributes.getBoolean(R.styleable.CropImageView_showThirds, false);
    ...
    public void setStyles(GridMode gridMode, boolean showCircle, int highlightColor, HandleMode handleMode){
    ...
    protected void draw(Canvas canvas) {
        ...
        if (!hasFocus()) {
            ...
            if (gridMode == GridMode.Always ||
                    (gridMode == GridMode.Changing && modifyMode == ModifyMode.Grow)) {
            	  drawGrid(canvas);
            }
    ...
    public int getHit(float x, float y) {
        ...
        final float hysteresis = 20F;//裁剪框四边touch响应区域宽度
        ...
        if ((Math.abs(r.left - x)     < hysteresis)  &&  verticalCheck) {
            retval |= GROW_LEFT_EDGE;//靠左边界
        }
        if ((Math.abs(r.right - x)    < hysteresis)  &&  verticalCheck) {
            retval |= GROW_RIGHT_EDGE;//靠右边界
        }
        if ((Math.abs(r.top - y)      < hysteresis)  &&  horizCheck) {
            retval |= GROW_TOP_EDGE;//靠上边界
        }
        if ((Math.abs(r.bottom - y)   < hysteresis)  &&  horizCheck) {
            retval |= GROW_BOTTOM_EDGE;//靠下边界
        }
    ...
    public Rect getScaledImageRect(float scale) {
        return new Rect((int) (imageRect.left * scale), (int) (imageRect.top * scale),
                (int) (imageRect.right * scale), (int) (imageRect.bottom * scale));
    }
